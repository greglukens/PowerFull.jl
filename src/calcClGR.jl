...
# =============================================================================
#
# Standalone module for computing C_ℓ^GR from pre-computed integrals.
#
# Requires: JLD2 only
...
function tracer_to_clgr_params(tracer::Tracer, cfns;
                                ...
                                bg_scale::Union{Nothing,Function} = nothing,
                                Q_scale::Union{Nothing,Function} = nothing)::ClGRParams
    D_fn   = r -> cfns.fDr(r)
    f_base = r -> cfns.ffr(r)
    Om_fn  = r -> cfns.fOmr(r)
    a_fn   = r -> cfns.far(r)
    ...
    # polynomial nuisance scheme).  f_scale(z), bg_scale(z) default to 1 (no
    # change), so the fiducial call is byte-identical to before.  fσ8 ∝ f and
    # bσ8 ∝ bg (σ8 a constant), so rescaling fσ8/bσ8 ≡ rescaling f/bg.  The f
    # modulation flows into β, B, A; bg modulation into bg, β, B/β, A/β, bΦ.
    ...
    # Optional multiplicative modulation of 𝒬(z) (magnification-bias nuisance,
    # analogous to the Khek et al. fσ8/bσ8 polynomial scheme).  Q_scale(z)
    # defaults to 1 (no change), so the fiducial call is byte-identical to
    # before.  The modulated Q_fn flows into EVERY place 𝒬 enters the kernel:
    #   - C_fn   (the (1-𝒬) and -2𝒬 terms of 𝒞)
    #   - A_fn   (the -2𝒬 term of 𝒜)
    #   - the stored ClGRParams.Q, used at contraction for the (1-𝒬) prefactors
    #     on the time-delay and lensing contributions.
    # Note bPhi depends on bg only, so 𝒬 modulation correctly does NOT touch the
    # PNG bias (no spurious f_NL coupling through this path).
    Q_fn  = if Q_scale === nothing
        r -> tracer.Q(z_fn(r))
    else
        r -> tracer.Q(z_fn(r)) * Q_scale(z_fn(r))
    end
    β_fn  = r -> f_fn(r) / bg_fn(r)
    ...
    B_fn = r -> f_fn(r) * (be_fn(r) + C_fn(r) - 1)

    function A_fn(r)
        Om = Om_fn(r); f = f_fn(r); Q = Q_fn(r)
        be = be_fn(r); C = C_fn(r)
        # Eq. (101) of Lukens & Jeong (2025), implemented VERBATIM — the fully
        # general 𝒜 with no ΛCDM assumption.  The expansion-history dependence
        # enters only through 𝒞 (which carries the general d ln H / d ln a via
        # C_fn); every explicit Ω_m here is a Poisson/density-relation factor,
        # not the Hubble-friction term.
        #
        #   𝒜 = (3/2)Ω_m [ b_e(1 - 2f/(3Ω_m)) + 1 + 2f/Ω_m + 𝒞 - f - 2𝒬 ]
        #
        # NOTE: we deliberately do NOT use the factored Eq. (3.51) form
        # (B/f)(1-2f/3Ω_m) + 2(1-Q)(1+2f/3Ω_m) - (4/3)(1-Q)/(ℋr) f/Ω_m.
        # That factored form is algebraically equal to this ONLY in ΛCDM: the
        # collapse to (3/2)Ω_m silently substitutes d ln H/d ln a = -(3/2)Ω_m
        # for a friction term sitting OUTSIDE 𝒞, so it re-bakes ΛCDM even when
        # C_fn carries the general d ln H/d ln a.  The two differ by
        # -f(3/2 Ω_m + d ln H/d ln a), which vanishes only in ΛCDM.
        return 1.5 * Om * (be * (1 - 2f / (3Om)) + 1 + 2f / Om + C - f - 2Q)
    end

    # PNG scale-dependent-bias convention:
    #
    # The standard Dalal/Slosar/Khek formula uses the growth factor normalized
    # to D(a) -> a in matter domination.  PowerFull's cosmology table usually
    # stores D_today with D(z=0)=1.  Therefore compute the constant conversion
    #
    #     g0 = D_MD(z=0) / D_today(z=0)
    #
    # from the table itself.  Since D_today/a tends to 1/g0 at high z, estimate
    # g0 = 1 / median[D/a] at the high-z end of the valid table.
    #
    # Do NOT rescale D_fn globally; that would change the Gaussian clustering.
    # Only bPhi is divided by g0, so the PNG response uses bPhi/D_MD.
    function _png_growth_g0_from_cfns()
        z_hi = min(10.0, z_fn(last(cfns.fzr.t)))
        z_grid = collect(range(max(2.0, 0.5*z_hi), z_hi; length=64))
        ...
        for zz in z_grid
            ...
            aa = a_fn(rr)
            DD = D_fn(rr)
            if isfinite(aa) && isfinite(DD) && aa > 0 && DD > 0
                push!(ratios, DD / aa)
            end
        end

        isempty(ratios) && return 1.0

        sort!(ratios)
        med = ratios[cld(length(ratios), 2)]

        return (isfinite(med) && med > 0) ? (1.0 / med) : 1.0
    end

    png_growth_g0 = _png_growth_g0_from_cfns()

    bPhi_fn = if tracer.bPhi === nothing
        r -> 2 * delta_c * (bg_fn(r) - 1) / png_growth_g0
    else
        r -> tracer.bPhi(z_fn(r)) / png_growth_g0
    end

    if get(ENV, "POWERFULL_PRINT_PNG_GROWTH_NORM", "0") in ("1", "true", "TRUE", "yes", "YES")
        @info "PNG growth normalization" png_growth_g0=png_growth_g0
    end
...
function compute_Cl_observed_multi(meta_path::String,
                                    ...
                                    Q_scales::Union{Nothing,Vector}=nothing,
    ...
    for k in 1:n_tracers
        ...
        Qsc  = (Q_scales  === nothing) ? nothing : Q_scales[k]
        params[k] = tracer_to_clgr_params(tracers[k], cfns;
            fNL=fNL_eff, Omm0=Omm0, H0=H0, delta_c=delta_c,
            f_scale=f_scale, bg_scale=bgsc, Q_scale=Qsc)
    end
    ...
    # =====================================================================
    # Selection-function quadrature
    #
    # WINDOW_QUAD=native:
    #   Original production behavior:
    #       u[k] = phi(z(r_k)) * (dz/dr)_k * dr_k
    #
    # WINDOW_QUAD=zproj:
    #   Integrate phi directly on a dense tracer-adaptive z grid, map each
    #   z node to r(z), and linearly project its probability mass onto the
    #   two surrounding native-r nodes.
    #
    # Contracting the native C(r_i,r_j) matrix with these projected weights
    # is exactly equivalent to a dense-z quadrature using bilinear
    # interpolation of that same native C surface.
    #
    # This isolates the Step-3 window-resolution problem without changing
    # or rebuilding the Step-1/Step-2 integral products.
    # =====================================================================

    window_quad = Symbol(lowercase(get(ENV, "WINDOW_QUAD", "native")))
    window_quad in (:native, :zproj) ||
        error("WINDOW_QUAD must be native or zproj; got '$window_quad'")

    u_per_tracer = Vector{Vector{Float64}}(undef, n_tracers)

    if window_quad === :native

        # ---------------------------------------------------------------
        # Original native-r quadrature
        # ---------------------------------------------------------------
        dr = Vector{Float64}(undef, nr)
        dr[1]   = 0.5 * (rr[2] - rr[1])
        dr[end] = 0.5 * (rr[end] - rr[end-1])
        ...
        for t in 1:n_tracers
            ...
            if n <= 0 || !isfinite(n)
                error(
                    "tracer[$t]: non-positive/non-finite native selection " *
                    "norm n=$n"
                )
            end
            ...
            if abs(n - 1) > 0.05
                @info(
                    "tracer[$t]: native discrete selection norm = " *
                    "$(round(n, digits=6)); renormalizing"
                )
            end
        ...
        # ---------------------------------------------------------------
        # Dense-z quadrature projected onto native r nodes
        # ---------------------------------------------------------------
        nz = parse(Int, get(ENV, "WINDOW_NZ", "257"))
        nscan = parse(Int, get(ENV, "WINDOW_ZSCAN", "8193"))
        active_rtol = parse(
            Float64,
            get(ENV, "WINDOW_ACTIVE_RTOL", "1e-12")
        )

        nz >= 8 || error("WINDOW_NZ must be at least 8")
        nscan >= nz || error("WINDOW_ZSCAN must be >= WINDOW_NZ")

        verbose && @info(
            "Using dense-z projected selection quadrature: " *
            "WINDOW_NZ=$nz WINDOW_ZSCAN=$nscan " *
            "WINDOW_ACTIVE_RTOL=$active_rtol"
        )
        ...
        for t in 1:n_tracers
            ...
            # First locate the actual active support of this tracer window.
            # The tracer spline often spans the full survey z range even when
            # the individual selection function is extremely narrow.
            zscan = collect(
                range(tr.zmin, tr.zmax; length=nscan)
            )

            pscan = Vector{Float64}(undef, nscan)
            @inbounds for a in eachindex(zscan)
                pscan[a] = max(0.0, phi_t(zscan[a]))
            end

            pmax = maximum(pscan)

            (pmax > 0 && isfinite(pmax)) ||
                error("tracer[$t]: invalid phi maximum pmax=$pmax")

            active = findall(x -> x > active_rtol * pmax, pscan)
            isempty(active) &&
                error("tracer[$t]: no active phi support found")

            # Add a couple of scan cells as a guard against truncation.
            ia = max(first(active) - 2, 1)
            ib = min(last(active) + 2, nscan)

            zlo = zscan[ia]
            zhi = zscan[ib]

            zhi > zlo ||
                error("tracer[$t]: invalid z interval [$zlo,$zhi]")

            # Dense quadrature nodes in the active z interval.
            zq = collect(range(zlo, zhi; length=nz))

            # Trapezoidal dz weights.
            wz = Vector{Float64}(undef, nz)
            wz[1]   = 0.5 * (zq[2] - zq[1])
            wz[end] = 0.5 * (zq[end] - zq[end-1])

            @inbounds for a in 2:nz-1
                wz[a] = 0.5 * (zq[a+1] - zq[a-1])
            end

            # q[a] is probability mass in dz. There is no dz/dr factor
            # because this quadrature is already performed in z.
            q = Vector{Float64}(undef, nz)

            @inbounds for a in 1:nz
                q[a] = max(0.0, phi_t(zq[a])) * wz[a]
            end

            qnorm = sum(q)

            (qnorm > 0 && isfinite(qnorm)) ||
                error("tracer[$t]: invalid dense-z norm qnorm=$qnorm")

            q ./= qnorm

            # Project each dense-z node onto the two neighboring native-r
            # nodes. This implements linear interpolation in r without ever
            # constructing a refined Nr x Nr matrix.
            u = zeros(Float64, nr)
            outside_mass = 0.0
            ...
            @inbounds for a in 1:nz
                ...
                if r <= rr[1]
                    u[1] += qa
                    outside_mass += r < rr[1] ? qa : 0.0

                elseif r >= rr[end]
                    u[end] += qa
                    outside_mass += r > rr[end] ? qa : 0.0

                else
                    k = searchsortedlast(rr, r)
                    k = clamp(k, 1, nr-1)

                    lambda = (r - rr[k]) / (rr[k+1] - rr[k])
            ...
            (unorm > 0 && isfinite(unorm)) ||
                error("tracer[$t]: invalid projected norm unorm=$unorm")

            u ./= unorm
            u_per_tracer[t] = u

            if outside_mass > 1e-8
                @warn(
                    "tracer[$t]: projected probability outside native rr " *
                    "range = $outside_mass"
                )
            end
            ...
            if verbose
                neff = 1.0 / sum(abs2, u)
                @info(
                    "tracer[$t] z-projection: " *
                    "z=[$(round(zlo,digits=6))," *
                    "$(round(zhi,digits=6))], " *
                    "Neff_native=$(round(neff,digits=3))"
                )
            end
        end
    end
