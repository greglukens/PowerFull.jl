...
include(joinpath(@__DIR__, "calcClGR_MG_tiled.jl"))
using .CalcClGR
...
function main()
    ...
    if multi
        ...
        npair_spec = isempty(strip(opts[:pairs])) ? 0 :
            count(==(','), opts[:pairs]) + 1
        println("Pairs             : $npair_spec entries (list suppressed)")
    else
    ...
    println(
        "MODIFIED GRAVITY  : mu0=$(opts[:mu0]), Sigma0=$(opts[:Sigma0])"
    )
    println()
    ...
    if multi
        ...
        write_pair_groups = lowercase(
            get(ENV, "POWERFULL_WRITE_PAIR_GROUPS", "1")
        ) in ("1", "true", "yes", "on")
        ...
        if n_pairs <= 20
            ...
            println("  first five: ", pair_indices[1:5])
            println("  last five : ", pair_indices[end-4:end])
        end
        ...
        diagnostic_pairs = n_pairs <= 20 ?
            collect(1:n_pairs) :
            unique(vcat(
                collect(1:5),
                collect(max(1, n_pairs - 4):n_pairs),
            ))
        ...
        for p in diagnostic_pairs
            ...
            println(
                "  pair ($ti, $tj):  " *
                "ℓ=$(meta_ell_values[1]) → $(Cl_obs[1,p]),  " *
                "ℓ=$(meta_ell_values[end]) → $(Cl_obs[end,p])"
            )
        end
        ...
        h5open(opts[:output], "w") do f
            ...
            f["provenance/write_pair_groups"] = write_pair_groups ? 1 : 0
        end
    ...
    params_1 = tracer_to_clgr_params(
        t1,
        cf;
        fNL=fNL_compute,
        Omm0=opts[:Omm0],
