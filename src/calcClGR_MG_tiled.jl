...
# Modified-gravity helpers. GR-inert at mu0=Sigma0=0.
...
function tracer_to_clgr_params(tracer::Tracer, cfns;
    ...
    # Modified growth and growth rate when MG is active.
    ...
    # Potential coefficient. The MG branch is the local-limit
    # modified expression and reduces to the production GR expression at
    # mu0=Sigma0=0.
    function A_fn(r)
        Om = Om_fn(r)
        f0 = f_fn(r)
        ...
        dmu0 = mg.dmu0_r(r)
        dSig0 = mg.dSig0_r(r)
        dmu0p_over_H = mg.dmu0_prime_r(r) / aH
        dSig0p_over_H = mg.dSig0_prime_r(r) / aH

        return 1.5 * Om * (
              (Bbar + 2 - 2Q) * (1 + dmu0)
            - f0
            - (2f0 / (3Om)) * (be - 3)
            - (2dSig0p_over_H - dmu0p_over_H)
            - 2f0 * dSig0
            + f0 * dmu0^2
            - 2f0 * dmu0 * dSig0
        )
...
function compute_Cl_observed_multi(meta_path::String,
    ...
    # Build the MG background/growth model once and reuse it for every tracer.
    mg = ((mu0 != 0.0) || (Sigma0 != 0.0)) ?
        build_mg_model(cfns; mu0=mu0, Sigma0=Sigma0) : nothing

    if verbose && mg !== nothing
        println(
            "  [MG] local-limit modified gravity: " *
            "mu0=$mu0, Sigma0=$Sigma0; modified D0, f0 and A"
        )
        flush(stdout)
    end
    ...
    for k in 1:n_tracers
        ...
        params[k] = tracer_to_clgr_params(
            tracers[k],
            cfns;
            fNL=fNL_eff,
            Omm0=Omm0,
