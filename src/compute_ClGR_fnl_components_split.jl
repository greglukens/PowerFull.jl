...
function _parse_args()
    opts = Dict{Symbol,Any}(
        ...
        :window_cosmo_funcr  => nothing,
        :radial_cosmo_funcr  => nothing,
        :fNL                 => 0.0,
        :delta_c      => 1.686,
        :Omm0         => nothing,
        :H0           => nothing,
    ...
    for arg in ARGS
        ...
        elseif startswith(arg, "--window-cosmo-funcr=")
            opts[:window_cosmo_funcr] = String(split(arg, "=", limit=2)[2])
        elseif startswith(arg, "--radial-cosmo-funcr=")
            opts[:radial_cosmo_funcr] = String(split(arg, "=", limit=2)[2])
...
function main()
    ...
    window_cosmo_path = opts[:window_cosmo_funcr] === nothing ?
        opts[:cosmo_funcr] : opts[:window_cosmo_funcr]

    radial_cosmo_path = opts[:radial_cosmo_funcr] === nothing ?
        opts[:cosmo_funcr] : opts[:radial_cosmo_funcr]

    isfile(window_cosmo_path) ||
        error("window cosmology table not found: $window_cosmo_path")

    isfile(radial_cosmo_path) ||
        error("radial cosmology table not found: $radial_cosmo_path")
    ...
    println("Base cosmology table   : $(opts[:cosmo_funcr])")
    println("Window r(z) table      : $window_cosmo_path")
    println("Radial/background table: $radial_cosmo_path")
    ...
    println("Loading split cosmologies...")

    cf_window = cosmofn(window_cosmo_path)
    cf_radial = cosmofn(radial_cosmo_path)

    # Hybrid cosmology:
    #
    #   frz  comes only from the window table and controls z -> r window placement.
    #
    #   fzr, far, fHr, fOmr, ffr, fDr come only from the radial table and
    #   control inverse z(r), tracer/background evaluation, a, H, Omega_m, f, D.
    cf = cosmofn(
        cf_window.frz,
        cf_radial.fzr,
        cf_radial.far,
    ...
    println("  frz window source : $window_cosmo_path")
    println("  all radial splines: $radial_cosmo_path")
    ...
    if multi
        ...
        Cl_obs = @time compute_Cl_observed_multi(opts[:input], tracers, pair_indices,
            ...
            delta_c=opts[:delta_c], variant=opts[:variant], verbose=true)
        ...
        h5open(opts[:output], "w") do f
            ...
            f["provenance/window_cosmo_funcr"]  = window_cosmo_path
            f["provenance/radial_cosmo_funcr"]  = radial_cosmo_path
    ...
    params_1 = tracer_to_clgr_params(t1, cf;
        fNL=fNL_compute, Omm0=opts[:Omm0], H0=opts[:H0], delta_c=opts[:delta_c])
    params_2 = if t2 === t1
        params_1
    else
